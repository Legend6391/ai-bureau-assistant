...
document.addEventListener('DOMContentLoaded', () => {
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            // Check if URL hash indicates we started here
            if (window.location.hash.includes('documents')) {
                fetchAndDisplayDocuments();
                hasFetchedOnce = true;
            }
    ...
    const createDocumentCard = (docId, data) => {
        ...
        div.querySelector('.btn-delete-doc').addEventListener('click', async (e) => {
            ...
            if (confirm("Are you sure you want to permanently delete this document from your vault?")) {
                try {
                    const { error } = await supabase.from('documents').delete().eq('id', targetDocId);
                    if (error) throw error;
                    
                    document.getElementById(`doc-card-${targetDocId}`).remove();
                    // Refetch or check if empty manually
    ...
    const handleUploadEvent = async (file) => {
        ...
        try {
            ...
            const uploadResult = await uploadDocument(file, currentUser.uid);
